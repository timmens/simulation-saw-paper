...
warning("New simulation dont have weird first column!!!!")

# read simulation results
df_R <-
  readr::read_csv(file.path(r_path, "rsimulation-dgp1-FINAL.csv")) %>%
  select(-c(1, s_01, s_02, hd_mean, hd_sd))
...
df <-
  readr::read_csv(file.path(r_path, "rsimulation-dgp2-to-dgp6-FINAL.csv")) %>%
...
table5_R <- construct_df(df_dgp5)


## Tables 2 - 5 (matlab): data cleaning and construction for matlab codes
warning("New simulation go from dgp2-dgp5 not dgp6")
df <-
...
## Table 6 (R): data cleaning and construction
warning("New simulation dont have weird first column!!!!")
table7_R <-
  readr::read_csv(file.path(r_path, "rsimulation-dgp7-FINAL.csv")) %>%
  select(-c(1))
...
# save
xtable::print.xtable(t1_mise_latex, file=file.path(tex_path, "table1_mise.tex"), include.rownames = FALSE)

warning("table2-table6 not implemented yet.")
# construct_table(table2, digits = 2) %>%
#   add_header_above(header = c("DGP2" = 20))
# 
# construct_table(table3, digits = n_digits) %>%
#   add_header_above(header = c("DGP3" = 20))
# 
# construct_table(table4, digits = n_digits) %>%
#   add_header_above(header = c("DGP4" = 20))
# 
# construct_table(table5, digits = n_digits) %>%
#   add_header_above(header = c("DGP5" = 20))
# 
# construct_table(table6, digits = n_digits) %>%
